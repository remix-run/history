...
/***/ function(module, exports, __webpack_require__) {
	...
	function createHistory() {
	    ...
	    if (location) {
	      ...
	      processMiddleware(function (ok) {
	        ...
	        if (ok) {
	          ...
	          _invariant2['default'](false, 'The initial route location transition was aborted by middleware.');
	        }
	      });
	    }
	  ...
	  function registerTransitionMiddleware(handler) {
	    if (transitionMiddlewareHandlers.indexOf(handler) === -1) transitionMiddlewareHandlers.push(handler);
	    return function () {
	  ...
	  function unregisterTransitionMiddleware(handler) {
	    transitionMiddlewareHandlers = transitionMiddlewareHandlers.filter(function (item) {
	      return item !== handler;
	    });
	  }

	  function processMiddleware(callback, optionalMiddleware) {
	    var handlers = optionalMiddleware || transitionMiddlewareHandlers;
	    ...
	    // Run middleware.
	    var pendingLocationCache = pendingLocation;
	    handlers[0](pendingLocation, location, function (value) {
	      if (pendingLocationCache === pendingLocation) {
	        if (arguments.length === 0) {
	          ...
	          _invariant2['default'](false, 'Unexpected middleware value "%s"', value);
	        }
	      }
	    });
	  }
	  ...
	  return {
	    ...
	    registerTransitionMiddleware: registerTransitionMiddleware,
	    unregisterTransitionMiddleware: unregisterTransitionMiddleware,
